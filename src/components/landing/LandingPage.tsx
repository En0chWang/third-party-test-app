    ...
    const getItem = async () => {
      try {
        ...
        if (state) {
          ...
          const finalAmazonRedirectURI = new URL(
            decodedState.replace("3pstate", "")
          );

          // validate the url before using it for navigation
          if (
            finalAmazonRedirectURI.protocol !== "http:" &&
            finalAmazonRedirectURI.protocol !== "https:"
          ) {
            console.error("Invalid protocol in redirect URL");
            return;
          }
          ...
          {props.user && (
            <>
              <p className="font-weight-light">
                Hello {props.user.signInDetails?.loginId}
              </p>
            </>
          )}
        </Col>
