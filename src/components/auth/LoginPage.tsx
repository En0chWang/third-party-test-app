  ...
  const handleLogin = async () => {
    try {
      ...
      const amazon_callback_uri =
        queryParams.get("amazon_callback_uri") ||
        queryParams.get("amzn_callback_url");
      ...
      const thirdPartyReturnURI = encodeURIComponent(
        `https://${window.location.host}/landing`
      ); // 3p landing page
      const combinedThirdPartyState = encodeURIComponent(
